...
const AdminBranchPerformance = () => {
  ...
  // Check if user came from admin menu
  const fromAdminMenu = location.state?.fromNewAdminMenu;
  ...
  return (
    <div className="min-h-screen w-full bg-[#0D1B2A]">
      <div className="p-6">
        <div className="mb-8">
          <h1 className="text-white text-xl sm:text-2xl font-bold text-center">Admin Branch Performance</h1>
        </div>
        ...
        <div className="max-w-2xl mx-auto mb-4 space-y-2">
          ...
          <button
            onClick={() => navigate('/new-admin-menu')}
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-700">
                      ...
                      <h4 className="text-white font-semibold text-sm mt-4 mb-2">Department Distribution</h4>
                      <div className="space-y-1">
                        {Object.entries(branch.metrics.departments).map(([dept, count]) => (
                          <div key={dept} className="flex justify-between items-center">
                            <span className="text-gray-400 text-xs">{dept}</span>
                            <span className="text-blue-400 text-xs">{count} staff</span>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3 max-w-7xl mx-auto px-2 sm:px-4">
            {filteredStaff.map((staff) => (
              <div
                ...
                onClick={() => navigate(`/weekly-analysis/${staff.id}`, { state: { fromAdminDashboard: true } })}
              >
        ...
        {filteredStaff.length === 0 && filteredBranches.length === 0 && (
...
export default AdminBranchPerformance; 